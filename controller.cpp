...
const unsigned int ADDITIONAL_FRAMES = 50u;
                               ...
                               const unsigned int& DT, const bool makeVideo)
{
  ...
  while (!done) {
    ...
    if(update){
      ...
      if(freshFrame)
      {
        ...
        if(msgImg != NULL){
          draw(msgImg->getPixieImage(), screen, msgImg->getX() - (msgImg->getPixieImage()->w)/2, msgImg->getY() - msgImg->getPixieImage()->h);
        }
  ...
  for(unsigned int i = 0; i < ADDITIONAL_FRAMES; i++){
    if (makeVideo) {
      genFrames.makeFrame();
    }
  }
